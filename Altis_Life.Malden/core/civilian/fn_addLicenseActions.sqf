...
_licenseDealers = [license_shop_1, license_shop_2];
{
	...
	_x addAction[format ["<t color='#B1FF5E'>" + "%1 ($%2)",localize (getText(missionConfigFile >> "Licenses" >> "hunting" >> "displayName")), [(getNumber(missionConfigFile >> "Licenses" >> "hunting" >> "price"))] call life_fnc_numberText],life_fnc_buyLicense,"hunting",0,false,false,"",' !license_civ_hunting && playerSide isEqualTo civilian '];
	_x addAction[format ["<t color='#B1FF5E'>" + "%1 ($%2)",localize (getText(missionConfigFile >> "Licenses" >> "gun" >> "displayName")), [(getNumber(missionConfigFile >> "Licenses" >> "gun" >> "price"))] call life_fnc_numberText],life_fnc_buyLicense,"gun",0,false,false,"",' !license_civ_gun && playerSide isEqualTo civilian '];
