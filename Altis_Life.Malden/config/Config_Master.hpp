...
/* System Settings */
	...
	allow_Dead_Body_Looting = true; //Set to true to allow players to loot dead bodys. False (default) will stop players from looting dead bodys.
    ...
    news_broadcast_cost = 15000; //Cost for a player to send a news station broadcast.
    ...
    escapeMenu_displayText = "ETG Malden Life"; //Text displayed in the escape menu. Make it short.. around 20 characters.

    /* Fuel System Configurations */
    pump_service = true; //Allow users to use pump service on the map. Default = false
    fuel_cost = 2; //Cost of fuel per liter at fuel stations (if not defined for the vehicle already).
    service_chopper = 1000; //Cost to service chopper at chopper service station(Repair/Refuel).
    fuelCan_refuel = 40; //Cost to refuel an empty fuel canister at the fuel station pumps. (Be wary of your buy/sell prices on fuel cans to prevent exploits...)

    /* Gang System Configurations */
    gang_price = 12000; //Gang creation price. --Remember they are persistent so keep it reasonable to avoid millions of gangs.
    ...
    houseGarage_buyPrice = 15500;
    houseGarage_sellPrice = 4500;
	...
	animal_max = 20; //Maximum ammount of animals on the map (can very up to 5 from the actual ammount - affects performance...)
    ...
    vehicle_rental_multiplier_CIVILIAN = .35; //Civilian Vehicle Rental Price = Config_Vehicle price * multiplier
    ...
    vehicle_sell_multiplier_COP = .45; //Cop Vehicle Garage Sell Price = Vehicle Buy Price * multiplier
